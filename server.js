...
app.post("/signup",function(req,res){
    ...
    {
        return res.render('error',{
            message: 'Only nitw domains can register',
            code:'signup'
           });
    }
    ...
    db.query('SELECT * from student_credentials WHERE email = ?',[email],async function(err,result)
    {
        ...
        db.query('UPDATE student_credentials SET ? WHERE email = ?',[{
            ...
            if(error){
                ...
                sendMail(email,'Temporary password', pass + ' is your temporary password for nitwPfolio');
                console.log(result);
                return res.render('success',{
    ...
    db.query('SELECT email,req_type from admin_panel WHERE id = ?',[id],function(err,result)
        {
            if(err) {
               ...
               sendMail(email,'Congratulations', 'Dear User, your request to '+req_type+' is reviewed successfully. Flaunt your new skills to the world using nitwPfolio.');
